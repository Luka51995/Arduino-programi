//RFID
#include "SPI.h"
#include "MFRC522.h"

#define SS_PIN 10
#define RST_PIN 9
#define SP_PIN 8
MFRC522 rfid(SS_PIN, RST_PIN);

MFRC522::MIFARE_Key key;


const int LED_PIN = A0;
const int PREKIDAC = 2;
const int PREKIDAC2 = 4;
const int IN1 = 3;
const int IN2 = 5;
const int ENA = 6;
const int prekidaczatvori= 8;
const int prekidacotvori = 7;
int zatvoreno = 0;    //closed
int otvoreno = 0;     //open
//int STOP = 0; 
//int trenutnostanje = 0; //current state
int zatvori = 0;
int otvori =0;
void setup() {
  Serial.begin(9600);
  SPI.begin();
  rfid.PCD_Init();
  pinMode(IN1,OUTPUT);
  pinMode(IN2,OUTPUT);
  pinMode(ENA,OUTPUT);
  //digitalWrite (ENA, LOW);
  pinMode(LED_PIN,OUTPUT);
  pinMode (prekidacotvori,INPUT);
  pinMode (prekidaczatvori,INPUT);
  pinMode (PREKIDAC,INPUT);
  pinMode (PREKIDAC2,INPUT);
  zatvoreno = digitalRead(PREKIDAC); //check if "closed" switch is pressed
  otvoreno = digitalRead (PREKIDAC2); //check if "open" switch is pressed
  //zatvori = digitalRead (prekidaczatvori);
  //otvori = digitalRead (prekidacotvori);
}

void loop() {
  if (!rfid.PICC_IsNewCardPresent() || !rfid.PICC_ReadCardSerial())
    return;

  // Serial.print(F("PICC type: ")); ispisuje koji je PICC type
  MFRC522::PICC_Type piccType = rfid.PICC_GetType(rfid.uid.sak);
  // Serial.println(rfid.PICC_GetTypeName(piccType));

  // Check if the PICC of Classic MIFARE type- Služi samo za ispitivanje vrste kartice, ispisuje ako nije vrsta MIFARE classic
  if (piccType != MFRC522::PICC_TYPE_MIFARE_MINI &&
    piccType != MFRC522::PICC_TYPE_MIFARE_1K &&
    piccType != MFRC522::PICC_TYPE_MIFARE_4K) {
    Serial.println(F("Your tag is not of type MIFARE Classic."));
    return;
  }
  
  String strID = "";
  for (byte i = 0; i < 4; i++) {
    strID +=
    (rfid.uid.uidByte[i] < 0x10 ? "0" : "") +
    String(rfid.uid.uidByte[i], HEX) +
    (i!=3 ? ":" : "");
  }
  strID.toUpperCase(); //slova iz stringa u CAPS LOCK (velika slova)

  // kada ova varijabla strID zadrži UID od oznake (tag)
  // može se koristiti kao input u IF (It can be used as input to IF to the tube)
  // LED je ugašena ili upaljena (LED illuminated or extinguished)
  Serial.print("Tap card key: ");
  Serial.println(strID);
 
zatvori = digitalRead (prekidaczatvori);
otvori = digitalRead (prekidacotvori);
int ca = strID.indexOf ("75:AA:25:A6");
zatvoreno = digitalRead(PREKIDAC);
otvoreno = digitalRead (PREKIDAC2);

 //OTVARANJE I ZATVARANJE SA RFID-om
 
  if (ca >=0 && zatvoreno == HIGH){ //ako očita karticu i ako je "zatvoreni" prekidač pritisnut otvara vrata
    digitalWrite(IN1,HIGH);      //CCW rotation
    digitalWrite(IN2,LOW);
    analogWrite(ENA,80);
    digitalWrite(LED_PIN,HIGH);
    
    delay(5000);   
    analogWrite (ENA,30);
    delay(3000);
    analogWrite(ENA,0);
    digitalWrite(IN1, LOW);
    digitalWrite(LED_PIN,LOW); }


  if (ca >=0 && otvoreno == HIGH){ //ako očita karticu i ako je "otvoreni" prekidač pritisnut zatvara vrata
    digitalWrite(IN1,LOW);        //CW rotation
    digitalWrite (IN2,HIGH); 
    analogWrite(ENA,80);   
    digitalWrite(LED_PIN,HIGH);
    
    delay(5000);
    analogWrite (ENA,30);
    delay(3000);
    analogWrite(ENA,0);
    digitalWrite (IN2,LOW);
    digitalWrite(LED_PIN,LOW); }
    
    
//OTVARANJE I ZATVARANJE SA PREKIDAČIMA

 if (otvori == HIGH ){ //ako je pritisnuta tipka za otvaranje otvara vrata
    digitalWrite(IN1, HIGH);      //CCW rotation
    digitalWrite(IN2, LOW);
    analogWrite(ENA,80);
    digitalWrite(LED_PIN, HIGH);
    
    //delay(5000);   
    //analogWrite (ENA,30);

    zatvori = 0;
    delay(3000);
    digitalWrite (IN2, LOW);
    digitalWrite(IN1, LOW);
    digitalWrite(LED_PIN, LOW); 
    }


  if (zatvori == HIGH){ //ako je pritisnuta tipka za zatvaranje zatvara vrata
    digitalWrite(IN1, LOW);        //CW rotation
    digitalWrite (IN2, HIGH); 
    analogWrite(ENA,80);   
    digitalWrite(LED_PIN, HIGH);
    
    //delay(5000);
    //analogWrite (ENA,30);

    otvori = 0;
    delay(3000);
    digitalWrite (IN1, LOW);
    digitalWrite (IN2, LOW);
    digitalWrite(LED_PIN, LOW); 
    }
    otvori = 0;
    zatvori = 0;
}
  
   
   
  




